...
const fourSum = (nums, target, result = [], numsIncluded = [], quads = new Set()) => {
  if (numsIncluded.length > 4) return result;

  if (satisfiesPredicate(numsIncluded, target)) {
    const sortedNums = numsIncluded.sort((a, b) => a - b);
    const sortedNumsString = sortedNums.join();
    if (!quads.has(sortedNumsString)) {
      quads.add(sortedNumsString);
  ...
  fourSum(rest, target, result, [ ...numsIncluded ], quads);
  fourSum(rest, target, result, [ ...numsIncluded, num ], quads);

  return result;
}

const satisfiesPredicate = (nums, target) =>
  nums.length === 4 && nums.reduce((sum, num) => sum + num) === target;
